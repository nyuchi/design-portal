...
import { SEED_COMPONENT_SPECS, SEED_ACCESSIBILITY } from "@/lib/brand"
...
<div className="mt-6 grid gap-4 sm:grid-cols-3">
  <div className="rounded-xl border border-border p-4">
    <p className="font-mono text-2xl font-bold text-foreground">{SEED_ACCESSIBILITY.minTouchTarget}px</p>
  ...
  <div className="rounded-xl border border-border p-4">
    <p className="font-mono text-2xl font-bold text-foreground">{SEED_ACCESSIBILITY.standard}</p>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="border-t border-border pt-3">
          <code className="font-mono text-xs text-muted-foreground">
            npx shadcn@latest add https://design.nyuchi.com/api/v1/ui/{spec.name}
          </code>
        </div>
      </CardContent>
    </Card>
...
<p className="mt-2 text-sm text-muted-foreground">
  All interactive elements maintain a minimum {SEED_ACCESSIBILITY.minTouchTarget}px touch target
  ...
  {[24, 32, 36, 48].map((size) => (
    <div key={size} className="flex flex-col items-center gap-2">
      <div className="relative">
        {size < SEED_ACCESSIBILITY.minTouchTarget && (
          <div
            ...
            style={{ width: `${SEED_ACCESSIBILITY.minTouchTarget}px`, height: `${SEED_ACCESSIBILITY.minTouchTarget}px` }}
          />
        )}
        <div
...
<p className="mt-4 text-xs text-muted-foreground">
  Dashed outline shows the {SEED_ACCESSIBILITY.minTouchTarget}px minimum hit area for elements smaller than the target.
</p>
