...
These resilience patterns are critical for African infrastructure where connectivity is unreliable. The mukoko ecosystem operates across seven data layers — Pod (Web3), Relational (Supabase), Document (ScyllaDB), Sync (CouchDB), Edge (Cloudflare KV + Durable Objects), Device (RxDB/SQLite), and Open Data (Doris) — and resilience must work across all of them. Circuit breakers protect cross-layer calls, retry handles transient failures at the edge, fallback chains cascade from fast edge caches down to sovereign databases. Speed is rented (edge caches). Truth is owned (sovereign databases).
...
npx shadcn@latest add https://design.nyuchi.com/api/v1/ui/fallback-chain
npx shadcn@latest add https://design.nyuchi.com/api/v1/ui/timeout
```
